...
export default function Layout() {
  ...
  const { cartCount, userRole, setUserRole, theme, setTheme } = useApp();
    ...
    <div className="flex h-screen bg-background text-foreground transition-colors duration-300 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-card border-b border-border px-6 py-3 flex items-center justify-between transition-colors duration-300">
          {/* Left: Role Switcher & Theme Toggle */}
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-[10px] tracking-widest text-gray-500 uppercase font-semibold">Rol:</label>
              <select
                value={userRole}
                onChange={(e) => setUserRole(e.target.value as any)}
                className="bg-background border border-border rounded-lg px-2.5 py-1 text-xs text-foreground font-semibold focus:outline-none focus:border-[#D4AF37] cursor-pointer"
              >
          ...
          {/* Right: Cart shortcut & User initials */}
          <div className="flex items-center gap-5">
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#F4D03F] flex items-center justify-center cursor-pointer">
              <span className="text-black text-xs font-semibold uppercase">
                {userRole === "administrador" ? "AD" : userRole === "cajera" ? "CA" : "ME"}
              </span>
            </div>
          </div>
        </header>
